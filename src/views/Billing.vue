...
export default {
  ...
  data() {
    return {
      bills: [], // Aquí se almacenan las facturas obtenidas
    };
  },
  ...
  async mounted() {
    setTooltip();
    try {
      const response = await getAllBillings(); // Llamada al servicio
      this.bills = response.data; // Asigna los datos al array bills
    } catch (error) {
      console.error("Error cargando las facturas:", error); // Manejo de errores
    }
  },
};
</script>
