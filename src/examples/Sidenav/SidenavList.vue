  ...
  >
    <ul class="navbar-nav">
      ...
      <li v-if="isUser" class="nav-item">
        <sidenav-collapse navText="Cuenta" :to="{ name: 'Billing' }">
          <template #icon>
            <credit-card />
          </template>
        </sidenav-collapse>
...
export default {
  ...
  created() {
    // Simulación: Obtener el rol desde el token o API
    const token = localStorage.getItem("authToken");
    if (token) {
      const payload = JSON.parse(atob(token.split(".")[1]));
      this.userRole = payload.role; // Suponiendo que el rol viene en el token
    }
  },
};
</script>
